...
<div class="px-6 py-4">
  <div class="flex justify-between">
    ...
    href="{{ route('admin.admins.tambah') }}">
    + Tambah
  </a>
  </div>
        ...
        <tbody class="divide-y divide-gray-100">
            @forelse($admin as $adm)
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $adm->name }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $adm->email }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $adm->password }}</td>
                    <td class="px-6 py-4">
                      <div class="flex items-center justify-center gap-2 text-sm">
                        {{-- Edit --}}
                        <a href="{{ route('admin.admins.edit', $adm->id_user) }}"
                          class="px-3 py-1 bg-gray-200 rounded text-xs text-gray-900">
                          Edit
                        </a>
                        {{-- Delete --}}
                        <form action="{{ route('admin.admins.destroy', $adm->id_user) }}" method="POST"
            ...
            @empty
                <tr>
                    <td colspan="4" class="px-6 py-8 text-center text-gray-500">Tidak ada Admin</td>
                </tr>
            @endforelse
        </tbody>
    </table>
